...
export async function runScan() {
  ...
  // Cap at 20 stocks per sector to stay within function timeout
  const MAX_PER_SECTOR = 20;
  const tickersToScan = [];
  for (const sector of positiveSectors) {
    tickersToScan.push(...getTickersInSector(sector).slice(0, MAX_PER_SECTOR));
  }

  console.log(`[Scanner] Scanning ${tickersToScan.length} stocks...`);

  // Step 3: Fetch historical data and evaluate (50ms delay — FMP has no strict rate limit)
  const historicalData = await fetchBatchHistorical(tickersToScan, 50);
